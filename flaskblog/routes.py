...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash('新たなアカウントの作成に成功しました。', 'success')
...
def login():
	...
	if form.validate_on_submit():
		...
		if user and bcrypt.check_password_hash(user.password,form.password.data):
			...
			flash('あなたは、ログインに失敗しました。もう一度アカウント認証をお願いします。')
...
def user_posts(username):
	page = request.args.get('page',1,type=int) #int型を指定
	user = User.query.filter_by(username=username).first_or_404() #項目名のみ
	posts = Post.query.filter_by(author=user).order_by(Post.date_posted.desc()).paginate(page=page,per_page=5) #特定の列の値に応じて行の並び替えを行うこと。
...
def reset_token(token):
	...
	if user in None:
		flash('これは無効な期限切れトークンです','warning')
